...
class VisualizationUtils:
    ...
    def draw_bbox(image: np.ndarray,
        ...
        if _KOREAN_FONT_PATH is not None:
            font_size = max(14, int(22 * viz_config.FONT_SCALE))
            try:
            ...
            pad = 4
            bg_top = max(0, y1 - th - pad * 2)
            draw.rectangle(
                [(x1, bg_top), (x1 + tw + pad * 2, y1)],
                fill=(color[2], color[1], color[0]),
            )
            draw.text(
                (x1 + pad - left, bg_top + pad - top),
                label,
                font=font,
                fill=(viz_config.TEXT_COLOR[2], viz_config.TEXT_COLOR[1], viz_config.TEXT_COLOR[0]),
            )
